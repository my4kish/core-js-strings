...
function removeFirstOccurrences(str, value) {
  ...
  //   n = str.search(value[i]);
  // }
  // return str.slice(0, n - value.length) + str.slice(n + 1, str.length);
...
function formatTime(minutes, seconds) {
  ...
  let secend = '';
  if (minutes < 10) {
    minend = min.padStart(2, '0');
  } else {
    minend = min;
  }
  if (seconds < 10) {
    secend = sec.padStart(2, '0');
  } else {
    secend = sec;
  }
  return `${minend}:${secend}`;
}
...
function countVowels(str) {
  let sum = 0;
  const lowStr = str.toLocaleLowerCase();
...
function isPalindrome(str) {
  ...
  if (revStr.replaceAll(' ', '') === newStr.replaceAll(' ', '')) {
    bool = true;
  } else {
    bool = false;
  }
...
function findLongestWord(sentence) {
  const str = sentence.split(' ').sort((a, b) => b.length - a.length);
  return str[0];
}
...
function extractNameFromTemplate(value) {
  return `${value.split(' ')[1]} ${value.replace('!', '').split(' ')[2]}`;
}
